  ...
  useEffect(() => {
    async function fetchListings(){
      try {
        ...
        const listingsRef = collection(db, "listings")
        // create the query
        const q = query(listingsRef, where("type", "==", "rent"), orderBy("timestamp", "desc"), limit(8));
        // execute the query
        const querySnap = await getDocs(q)
        const listings = []
        querySnap.forEach((doc) => {
  ...
  useEffect(() => {
    async function fetchListings(){
      try {
        ...
        const listingsRef = collection(db, "listings")
        // create the query
        const q = query(listingsRef, where("type", "==", "sale"), orderBy("timestamp", "desc"), limit(8));
        // execute the query
        const querySnap = await getDocs(q)
        const listings = []
        querySnap.forEach((doc) => {
       ...
       <div className='max-w-6xl mx-auto pt-4 space-y-6'>
        {offerListings && offerListings.length > 0 && (
          <div className='m-2 pb-6'>
            <h2 className='px-3 text-2xl mt-6font-semibold'>Recent offers</h2>
            <Link to="/offers">
        ...
        {rentListings && rentListings.length > 0 && (
          <div className='m-2 pb-6'>
            <h2 className='px-3 text-2xl mt-6font-semibold'>Places for rent</h2>
        ...
        {saleListings && saleListings.length > 0 && (
          <div className='m-2 pb-6'>
            <h2 className='px-3 text-2xl mt-6font-semibold'>Places for sale</h2>
